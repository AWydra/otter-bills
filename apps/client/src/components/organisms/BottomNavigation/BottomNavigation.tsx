...
function BottomNavigation() {
  ...
  useEffect(() => {
    ...
    if (location.pathname.includes(RouteEnum.ADD_RECEIPT)) {
      newValue = RouteEnum.ADD_RECEIPT;
    } else if (location.pathname.includes(RouteEnum.HISTORY)) {
      newValue = RouteEnum.HISTORY;
    }
    setValue(newValue);
  }, [location.pathname]);
  ...
  return !visible ? null : (
    <Paper elevation={3} sx={{ width: '100%', position: 'fixed', bottom: 0 }}>
      <MuiBottomNavigation
        showLabels
        value={value}
        onChange={(event, newValue: RouteEnum) => {
          setValue(newValue);
        }}
      >
        <BottomNavigationAction
